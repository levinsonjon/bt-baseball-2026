...
**The weekly Gmail re-auth Reminder was removed 2026-08-03** along with `check_gmail_reauth_needed()` and the `GMAIL_*` paths. Once this script stopped sending mail (2026-08-02) nothing in the active pipeline used that token, so it was nagging Jon to renew a credential only the retired `send_pending_email.py` would need. `daily_report.py` keeps its own separate copy of that check for `run_daily.py` — that one is still live and should not be removed.
