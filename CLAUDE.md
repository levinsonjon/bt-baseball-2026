...
- **⚠️ Drive/Sheets OAuth shared with the `gdrive-personal` MCP server (OPEN).** `update_health.py`'s `get_token()` uses the **gdrive** credential (it writes to Sheets), not the Gmail one. That credential lives in `~/.config/personal-mcp/gdrive/`, which the `gdrive-personal` MCP server also refreshes — the same racing-clients problem that revoked the Gmail token three times and was fixed 2026-05-12 by splitting to a dedicated `gmail-fb` client. Symptom: every 3:13am run dies at `get_token()` with `HTTP Error 400`, yet a manual refresh always succeeds, because the MCP server re-authorizes during the day and the token is revoked again by cron time. This silently killed the Sheets health/injury update for **24 days** (last success 2026-07-08, "Updated 72 cells", through 2026-08-01) — and it's invisible from the outside since `check_pipeline_freshness()` runs first and still fires, so the job looks like it's doing something. **Fix (not yet done, needs Jon in a browser):** create a second OAuth client ID (Desktop app) in GCP project `personal-claude-mcp-486922`, save to `~/.config/personal-mcp/gdrive-fb/gcp-oauth.keys.json`, run the auth flow. `update_health.py` prefers `gdrive-fb/` automatically whenever that keys file exists and otherwise falls back to `gdrive/` — no code change needed. A plain re-auth of the shared token revives it for a few days, then it breaks again.
